...
function pushH(key: string, val: number) {
...
function MetricsContent({ pixelH }: { pixelH: number }) {
  ...
  useEffect(() => {
    ...
    pushH('cpu.usage', cpu.usage_pct)
    const memPct = m.total_bytes > 0 ? (m.used_bytes / m.total_bytes) * 100 : 0
    pushH('mem.usage', memPct)
    gpus?.forEach((g, i) => pushH(`gpu.${i}.usage`, g.usage_pct))
    disks?.forEach(dk => {
      const pct = dk.total_bytes && dk.used_bytes !== undefined
        ? (dk.used_bytes / dk.total_bytes!) * 100 : 0
      pushH(`disk.${dk.name}.pct`, pct)
    })
    network?.forEach(n => pushH(`net.${n.name}.rx`, n.rx_bps))
    tick(n => n + 1)
  }, [res])
  ...
  const chartH = pixelH >= CELL_H * 4 ? 52 : pixelH >= CELL_H * 3 ? 32 : 0
    ...
    >
      ...
      {!res ? (
        ...
        <div className="flex flex-col gap-2 flex-1 overflow-hidden">
          ...
          {disks.filter(dk => dk.total_bytes && dk.total_bytes > 0 && dk.used_bytes !== undefined).map(dk => {
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-1 shrink-0">
            {cpu?.temp_c !== undefined && cpu.temp_c > 0 && (
              <span className="flex items-center gap-1 text-[10px] text-white/40">
                ...
                <span style={{ color: cpu.temp_c >= 80 ? '#ef4444' : cpu.temp_c >= 65 ? '#f59e0b' : undefined }}>
                  {cpu.temp_c.toFixed(0)}°C
                </span>
              </span>
            )}
...
export function MetricsWidget({ id }: { id: string }) {
  const pixelH = useWidgetStore(s => s.widgets.find(w => w.id === id)?.h ?? 192)
  return (
    <WidgetShell id={id}>
      <MetricsContent pixelH={pixelH} />
    </WidgetShell>
  )
}
